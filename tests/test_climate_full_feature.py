...
from aioesphomeapi.api_pb2 import (
    ClimateMode, ClimateFanMode, ClimatePreset, ClimateAction,
    ListEntitiesClimateResponse, ClimateStateResponse, ClimateCommandRequest
)

logging.basicConfig(level=logging.DEBUG)

class AdvancedClimateEntity(ClimateEntity):
    DOMAIN = "climate"
    ...
    async def set_state_from_command(self, command: ClimateCommandRequest):
        ...
        if command.has_target_temperature:
            old_temp = self.target_temperature_high if self.mode == ClimateMode.CLIMATE_MODE_COOL else self.target_temperature_low
            new_temp = command.target_temperature
            if self.mode == ClimateMode.CLIMATE_MODE_COOL:
                self.target_temperature_high = new_temp
            elif self.mode == ClimateMode.CLIMATE_MODE_HEAT:
                self.target_temperature_low = new_temp
            changed = True
            logging.info(f"  Target temperature changed: {old_temp:.1f}°C -> {new_temp:.1f}°C")
        ...
        if command.has_preset:
            ...
            changed = True
            logging.info(f"  Preset changed: {ClimatePreset.Name(old_preset)} -> {ClimatePreset.Name(self.preset)}")
    ...
    async def notify_state_change(self):
        ...
        logging.info(f"  Preset: {ClimatePreset.Name(self.preset)}")
...
async def run_device(name, api_port, web_port):
    ...
    device = Device(
        ...
        model="Test_Advanced Climate",
        project_name="aioesphomeserver",
        project_version="1.0.0",
    )

    climate_entity = AdvancedClimateEntity(
        device=device,
        name="Test_Advanced Climate",
        object_id="Test_Advanced_climate",
...
async def main():
    name, api_port, web_port = "Test_Advanced Climate Device", 6053, 8080
